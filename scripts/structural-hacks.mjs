 ...
 * pokeemerald-expansion source AFTER apply-story.mjs (which only changes text).
 *
 ...
 *
 * Every edit locates a verified anchor and fails loud if it's missing, so an
 * upstream pokeemerald-expansion change can never silently produce a vanilla ROM.
 *
 * Usage: node scripts/structural-hacks.mjs <expansionRoot>
 */
import fs from "node:fs";
import path from "node:path";

const expansionRoot = process.argv[2] || path.join("hack", "pokeemerald-expansion");
const errors = [];
let editCount = 0;
...
function replaceOnce(src, find, replacement, tag) {
...
if (errors.length > 0) {
  console.error("::error::Structural hacks incomplete — aborting so the build can't silently ship vanilla gameplay.");
  process.exit(1);
}
